...
[cite_start]_(Note for Mac users: Install via Homebrew using `brew install git-lfs`, then run `git lfs install`.)_ [cite: 172, 173, 174]
...
## Note: Small Unity team tip

For projects using Unity Editor, it's common to also run once:

```bash
git lfs pull
```

This ensures all LFS assets are downloaded correctly.
